...
public class FullConfig {
    ...
    public void load(JavaPlugin plugin) {
        try {
            ...
            // Server
            if (!new File(plugin.getDataFolder(), "config/bettercombat/server.yml").exists()) {
                ...
                this.server.reset_hits_after_miss = sc.getBoolean("reset_hits_after_miss", this.server.reset_hits_after_miss);
            ...
            this.save(plugin, true, true);
        }  catch (Exception e) {
            BetterCombatMod.LOGGER.trace("Config loading failed", e);
        }
    }

    public void save(JavaPlugin plugin, boolean server, boolean fallback) {
        try {
            new File(plugin.getDataFolder(), "config/bettercombat").mkdirs();
            if (server) {
                BetterCombatMod.LOGGER.info("Saving \"server\" config");
        ...
        try {
            if (fallback) {
                BetterCombatMod.LOGGER.info("Saving \"fallback\" config");
    ...
    public void saveServer(YamlConfiguration sc) {
        ...
        sc.set("reset_hits_after_miss", this.server.reset_hits_after_miss);
